...
max_iterations = 1000
test_size = 200
plot_hist = False   # Toggle to plot histograms in Tensorboard
plot_every = 100    # After how many iterations do you want output?
...
with tf.Session() as sess:
  print('Start session')
  writer = tf.train.SummaryWriter("/home/rob/Dropbox/ml_projects/FCN/log_tb", sess.graph_def)
  ...
  for i in range(max_iterations):
    ...
    if i%plot_every == 1:
      ...
      result = sess.run([cost,accuracy,merged,train_step],feed_dict={x:X_train[batch_ind], y_: y_train[batch_ind], keep_prob: 0.5, bn_train: True})
      perf_collect[0,step] = result[0]
      perf_collect[2,step] = result[1]
        
      #Write information for Tensorboard
      #Note: I prefer to take summaries at testtime, when the test-data is evaluated. 
      # For some reason, when I set bn_train: False, the summary op returns an error
      # for the meantime, we solve this by getting summaries over the traindata
      ...
      # For some strange reason, the next line only runs for bn_train: True
      result = sess.run([cost,accuracy], feed_dict={ x: X_test[test_ind], y_: y_test[test_ind], keep_prob: 1.0, bn_train: False})
      ...
      sess.run(train_step,feed_dict={x:X_train[batch_ind], y_: y_train[batch_ind], keep_prob: 0.5, bn_train: True})
